...
import { LayoutDashboard, SlidersHorizontal, MessageSquare, ShieldCheck, FileText, Bell, LogOut, Globe, Moon, Sun } from 'lucide-react'
...
const MainApp = () => {
  ...
  const navItems = [
    ...
    { id: 'subsidy', label: 'Subsidy Matcher', icon: ShieldCheck },
    { id: 'documents', label: 'Doc Automation', icon: FileText },
  ]
    ...
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <aside className="w-72 bg-background-surface border-r border-white/5 flex flex-col z-20">
         ...
         <div className="flex-1 overflow-y-auto py-6 px-4 space-y-2">
            ...
            {navItems.map((item) => (
               <button
                 ...
                 className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 relative group ${activeView === item.id ? 'text-white' : 'text-text-muted hover:text-white hover:bg-white/5'}`}
               >
                 <item.icon className={`w-5 h-5 ${activeView === item.id ? 'text-primary' : 'group-hover:text-primary transition-colors'}`} />
         ...
         <div className="p-4 border-t border-white/5 space-y-2">
            <div className="bg-background-layered p-4 rounded-xl border border-white/5 flex flex-col gap-1">
            ...
            <div className="flex gap-2 mt-2">
               ...
               <button onClick={logout} className="flex-1 h-10 rounded-lg bg-white/5 hover:bg-destructive/20 hover:text-destructive flex items-center justify-center text-text-muted transition-colors">
         ...
         <div className="flex-1 overflow-y-auto p-4 sm:p-8">
            <div className="max-w-6xl mx-auto pb-10">
               <AnimatePresence mode="wait">
                  ...
                  >
                     {activeView === 'dashboard' && <DashboardAnalytics business={businessData} />}
                     {activeView === 'simulator' && <WhatIfSimulator />}
                     {activeView === 'ai' && <AiAssistant />}
                     {activeView === 'subsidy' && <SubsidyChecker />}
                     {activeView === 'documents' && <DocumentAutomation />}
                  </motion.div>
               </AnimatePresence>
            </div>
         </div>
